...
from .limits import (
    ...
    EXECUTE_RPM_PER_ORG as _DEFAULT_EXECUTE_RPM,
    EXECUTE_TIMEOUT_MAX as _DEFAULT_EXECUTE_TIMEOUT,
    GUEST_DAILY_TOKEN_BUDGET as _DEFAULT_GUEST_BUDGET,
    GUEST_DAILY_TOKEN_BUDGET_PER_IP as _DEFAULT_GUEST_IP_BUDGET,
    GUEST_MAX_INPUT_TOKENS as _DEFAULT_GUEST_MAX_INPUT,
    GUEST_RPM_PER_USER as _DEFAULT_GUEST_RPM_USER,
    PASSWORD_MIN_LEN as _DEFAULT_PASSWORD_MIN,
    PBKDF2_ITERATIONS as _DEFAULT_PBKDF2_ITERATIONS,
    SESSION_TTL_SECONDS as _DEFAULT_SESSION_TTL,
    USERNAME_MAX_LEN as _DEFAULT_USERNAME_MAX,
    USERNAME_MIN_LEN as _DEFAULT_USERNAME_MIN,
)
...
#: 游客（未登录）额度。刻意压得很低——访客没有身份成本，清 cookie 就能重来，
#: 所以「够体验、不够白嫖」。详见 app/limits.py 的模块文档。
GUEST_DAILY_TOKEN_BUDGET = _env_int_allow_zero("GUEST_DAILY_TOKEN_BUDGET", _DEFAULT_GUEST_BUDGET)
GUEST_RPM_PER_USER = _env_int_allow_zero("GUEST_RPM_PER_USER", _DEFAULT_GUEST_RPM_USER)
GUEST_MAX_INPUT_TOKENS = _env_int_allow_zero("GUEST_MAX_INPUT_TOKENS", _DEFAULT_GUEST_MAX_INPUT)

#: 每个 IP 每天的游客 token 上限。**这一层才是真兜底**：游客清 cookie 换身份
#: 绕得过按身份的额度，绕不过按 IP 的。0 = 不限。
GUEST_DAILY_TOKEN_BUDGET_PER_IP = _env_int_allow_zero(
    "GUEST_DAILY_TOKEN_BUDGET_PER_IP", _DEFAULT_GUEST_IP_BUDGET
)

#: 账号规则。用户名唯一性判定是大小写不敏感的（注册时按小写归一）。
USERNAME_MIN_LEN = _env_int("USERNAME_MIN_LEN", _DEFAULT_USERNAME_MIN)
USERNAME_MAX_LEN = _env_int("USERNAME_MAX_LEN", _DEFAULT_USERNAME_MAX)
PASSWORD_MIN_LEN = _env_int("PASSWORD_MIN_LEN", _DEFAULT_PASSWORD_MIN)
PBKDF2_ITERATIONS = _env_int("PBKDF2_ITERATIONS", _DEFAULT_PBKDF2_ITERATIONS)
SESSION_TTL_SECONDS = _env_int("SESSION_TTL_SECONDS", _DEFAULT_SESSION_TTL)
